...
export default function RatingsPage() {
  ...
  useEffect(() => {
    if (!mounted) return;
    gsap.fromTo(navRef.current, { y: -20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.8, ease: "power3.out" });
    gsap.fromTo(headerRef.current, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.8, delay: 0.3, ease: "power3.out" });
  }, [mounted]);
  ...
  const avgRating = ratedCards.length > 0
    ? (ratedCards.reduce((sum, c) => sum + c.rating, 0) / ratedCards.length).toFixed(1)
    : "0.0";
    ...
    <div className="min-h-screen bg-[#050505] text-white">
          ...
          {[
            ...
            <SoundLink
              ...
              className={`nav-item text-[6px] md:text-[9px] uppercase tracking-widest transition-colors duration-300 relative group ${
                item.name === "Rate Me" ? "text-red-500" : "text-zinc-500 hover:text-red-500"
              } ${item.hideOnMobile ? "hidden md:block" : ""}`}
            >
              {item.name}
              <span className={`absolute -bottom-1 left-0 h-[1px] bg-red-600 transition-all duration-300 ${item.name === "Rate Me" ? "w-full" : "w-0 group-hover:w-full"}`} />
            </SoundLink>
          ))}
        </div>
      </nav>
      ...
      <div className="pt-16 md:pt-32 pb-4 md:pb-10">
        <div className="container mx-auto px-4 md:px-12 lg:px-24">
          <div ref={headerRef} className="ratings-header mb-4 md:mb-8 opacity-0">
            <div style={pixelFont} className="text-zinc-600 text-[8px] md:text-[10px] tracking-widest mb-2 md:mb-4 uppercase flex items-center gap-2">
              <Star size={12} className="md:w-[14px] md:h-[14px]" /> Rating_Board / {pastedCards.length} Ratings
            </div>
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      ...
                      fill={star <= Math.round(parseFloat(avgRating)) ? "#ef4444" : "transparent"}
                      stroke={star <= Math.round(parseFloat(avgRating)) ? "#ef4444" : "#3f3f46"}
                      strokeWidth={1.5}
                    />
                  ))}
                </div>
            ...
            <div className="flex items-center gap-3 md:gap-4 mt-2 md:mt-4">
              <div className="flex items-center gap-1.5 md:gap-2 text-zinc-500">
                ...
                <span style={pixelFont} className="text-[7px] md:text-[10px]">{pastedCards.length} TOTAL</span>
              </div>
              <div className="flex items-center gap-1.5 md:gap-2 text-zinc-500">
                <Star size={12} className="md:w-[14px] md:h-[14px]" fill="#ef4444" className="text-red-500" />
                <span style={pixelFont} className="text-[7px] md:text-[10px]">{ratedCards.length} RATED</span>
              </div>
            </div>
          </div>
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p style={pixelFont} className="text-[9px] text-zinc-500">Loading...</p>
            </div>
          )}
      ...
      <button
        ...
        style={pixelFont}
      >
        <Plus size={18} className="md:w-[20px] md:h-[20px] group-hover:rotate-90 transition-transform" />
