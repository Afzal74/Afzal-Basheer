...
export default function Home() {
  ...
  const logoShowcaseRef = useRef(null);
  ...
  useEffect(() => {
    ...
    if (hasAnimated) {
      ...
      // Show content after hero animation (4 seconds)
  ...
  // Sequential animation: LogoShowcase → Projects → Achievements
  useEffect(() => {
    ...
    if (!hasAnimated) {
      // Set initial states
      gsap.set(logoShowcaseRef.current, { opacity: 0, y: 30 });
      gsap.set(homeProjectsRef.current, { opacity: 0, y: 30 });
      gsap.set(achievementsRef.current, { opacity: 0, y: 30 });

      // Create sequential timeline
      const tl = gsap.timeline({ defaults: { ease: "power3.out" } });

      // 1. LogoShowcase appears first
      tl.to(logoShowcaseRef.current, { opacity: 1, y: 0, duration: 0.8 });

      // 2. Projects appears next
      tl.to(
  ...
  return (
    <main className="bg-[#050505] min-h-screen">
      ...
      {/* Content - shown after hero splash */}
      {showContent && (
        <>
          <div ref={logoShowcaseRef}>
            <LogoShowcase />
          </div>

          <div ref={homeProjectsRef}>
            <HomeProjects />
          </div>
          ...
          >
            ...
            >
              ...
              <img
                src={
                  mobileAchievements[activeAchievement].carouselImages
                    ? mobileAchievements[activeAchievement].carouselImages[
                        carouselIndex
                      ]
                    : mobileAchievements[activeAchievement].img
                }
              ...
              {mobileAchievements.map((ach, i) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    ...
                    <span
                      style={pixelFont}
                      className="text-[5px] text-zinc-600"
                    >
                      {ach.year}
                    </span>
                  </div>
