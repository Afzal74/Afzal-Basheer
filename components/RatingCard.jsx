...
const getCardWidth = () => (typeof window !== "undefined" && window.innerWidth < 640 ? 160 : 200);
...
const getProtectedZones = () => (typeof window !== "undefined" && window.innerWidth < 640 ? PROTECTED_ZONES_MOBILE : PROTECTED_ZONES_DESKTOP);
...
export default function RatingCard({ card, onUpdate, onDelete, isMobileGrid, mobileIndex }) {
  ...
  useEffect(() => {
    ...
    const id = card.id;
    gsap.to(`.border-top-${id}`, { left: "100%", duration: 3, repeat: -1, ease: "none" });
    gsap.to(`.border-right-${id}`, { top: "100%", duration: 3, repeat: -1, ease: "none", delay: 0.75 });
    gsap.to(`.border-bottom-${id}`, { right: "100%", duration: 3, repeat: -1, ease: "none", delay: 1.5 });
    gsap.to(`.border-left-${id}`, { bottom: "100%", duration: 3, repeat: -1, ease: "none", delay: 2.25 });
  }, [isPasted, card.id]);
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 opacity-15 pointer-events-none overflow-hidden">
          {[...Array(10)].map((_, i) => (
            <div key={i} className="absolute w-full h-px bg-blue-400" style={{ top: `${20 + i * 18}px` }} />
          ))}
        </div>
        ...
        <div
          ...
          style={{ background: "linear-gradient(180deg, rgba(255,255,200,0.9) 0%, rgba(255,255,180,0.7) 100%)", transform: "translateX(-50%) rotate(-1deg)" }}
        />

        {/* Header */}
        <div className="flex items-center justify-between px-2 py-1.5 border-b border-amber-200/50">
          <GripVertical size={12} className={isPasted ? "text-gray-300" : "text-amber-600/50"} />
          {isPasted ? (
            <button onClick={handleDeleteClick} className="no-drag p-0.5 hover:bg-red-100 rounded">
              <Trash2 size={12} className="text-red-400" />
            </button>
          ) : (
            <button onClick={() => onDelete(card.id)} className="no-drag p-0.5 hover:bg-red-100 rounded">
        ...
        {showDeletePrompt && (
          <div className="absolute inset-0 bg-black/85 z-30 flex flex-col items-center justify-center p-3 rounded">
            ...
            <input
              ...
              className={`no-drag w-full px-2 py-1 text-xs rounded border ${passwordError ? "border-red-500 bg-red-100" : "border-gray-300"}`}
              placeholder="Password..."
              autoFocus
            />
            {passwordError && <p className="text-red-400 text-[8px] mt-1">Wrong!</p>}
            <div className="flex gap-2 mt-2">
              <button onClick={() => setShowDeletePrompt(false)} className="no-drag px-2 py-1 text-[10px] bg-gray-600 text-white rounded">Cancel</button>
              <button onClick={handlePasswordSubmit} className="no-drag px-2 py-1 text-[10px] bg-red-600 text-white rounded">Delete</button>
            </div>
          </div>
        )}

        {/* Content */}
        <div className="p-2 sm:p-3 pt-1.5 sm:pt-2" onMouseDown={(e) => e.stopPropagation()}>
          <input
            type="text"
            value={localName}
          ...
          <input
            type="text"
            value={localMessage}
            onChange={(e) => { if (isPasted) return; const val = e.target.value.slice(0, 100); setLocalMessage(val); onUpdate({ ...card, message: val }); }}
            placeholder="Message..."
            maxLength={100}
            readOnly={isPasted}
            className="no-drag w-full bg-transparent border-none outline-none text-gray-600 text-[10px] sm:text-xs placeholder:text-gray-400 mt-0.5 cursor-text"
            style={{ fontFamily: "'Caveat', cursive" }}
          />
            ...
            <div className="flex gap-1 sm:gap-1.5 mb-1.5 sm:mb-2">
              {COLORS.map((color) => (
                <button
                  ...
                  className={`no-drag w-3 h-3 sm:w-4 sm:h-4 rounded-full hover:scale-110 transition-transform ${card.color === color ? "ring-2 ring-offset-1 ring-gray-400" : ""}`}
          ...
          {/* Paste button */}
          {!isPasted && (
            <button onClick={handlePaste} className="no-drag w-full py-1 sm:py-1.5 bg-green-500 hover:bg-green-600 text-white text-[10px] sm:text-xs font-medium rounded flex items-center justify-center gap-1 mb-1 sm:mb-1.5">
              <Check size={10} className="sm:w-3 sm:h-3" /> Paste
            </button>
          )}

          {/* Date */}
          <div className="text-[8px] sm:text-[10px] text-gray-500 pt-1 sm:pt-1.5 border-t border-amber-200/30" style={{ fontFamily: "'Caveat', cursive" }}>
            {card.date} • {card.time}
          </div>
        </div>
      </div>
